<template>
  <div class="content-wrapper">
     <div id="content">
        <section class="noprint content-header  " style="padding-left:10px;padding-top:10px">
              ...
              <div class="box box-default">
                  ...
                  <div class="box-body">
                      <table class="table table-bordered table-striped dataTable">
                        ...
                        <tbody>
                          <tr v-for="(row,idx) in showdata"  v-on:dblclick="showinfo(row)" >
                            <td>{{idx+1}}</td>
            ...
            <summary>&nbsp;
              <router-link  :to="'/depositpayment/1'" class="button" > View</router-link>
              <router-link  :to="'/depositpayment/edit/2'" class="button" >Edit</router-link>
              </summary>
            <footer>&nbsp;</footer>
        </section>
        <section class="print">
...
export default {
  ...
  //----------------------INIT -----
          const cols = [
            ...
            { key : 'name'         , readonly:0, default:'',inputtype: 'text', sortable: 0, visible: 1, classname:'left'   ,width: null, label: 'name'         },
            { key : 'email'        , readonly:0, default:'',inputtype: 'text', sortable: 0, visible: 1, classname:'left'   ,width: null, label: 'email'        },
            { key : 'tel'          , readonly:0, default:'',inputtype: 'text', sortable: 0, visible: 1, classname:'left'   ,width: null, label: 'tel'          },
            { key : 'status'       , readonly:0, default:'',inputtype: 'text', sortable: 0, visible: 1, classname:'center' ,width: null, label: 'status'       },
            { key : 'address1'     , readonly:0, default:'',inputtype: 'text', sortable: 0, visible: 0, classname:'left'   ,width: null, label: 'address1'     },
            { key : 'address2'     , readonly:0, default:'',inputtype: 'text', sortable: 0, visible: 0, classname:'left'   ,width: null, label: 'address2'     },
            { key : 'province'     , readonly:0, default:'',inputtype: 'text', sortable: 0, visible: 0, classname:'left'   ,width: null, label: 'province'     },
            { key : 'zipcode'      , readonly:0, default:'',inputtype: 'text', sortable: 0, visible: 0, classname:'left'   ,width: null, label: 'zipcode'      },
            { key : 'created_by'   , readonly:0, default:'',inputtype: 'text', sortable: 0, visible: 0, classname:'left'   ,width: null, label: 'created_by'   },
            { key : 'created_date' , readonly:0, default:'',inputtype: 'text', sortable: 0, visible: 0, classname:'left'   ,width: null, label: 'created_date' },
            { key : 'updated_by'   , readonly:0, default:'',inputtype: 'text', sortable: 0, visible: 0, classname:'left'   ,width: null, label: 'updated_by'   },
            { key : 'updated_date' , readonly:0, default:'',inputtype: 'text', sortable: 0, visible: 0, classname:'left'   ,width: null, label: 'updated_date' },
  ...
  //---------------------------
    return { 
        module: "Deposit Payments",
        header:'การจ่ายเงินมัดจำ',
        title:'รายการจ่ายเงินมัดจำ',
  ...
  computed:{
    ...mapGetters({datas:'branchs/branchs'}),
    totalpage() {
  ...
  methods:{
    ...
    showinfo(row){ 
        this.$router.push({name:'depositpaymentview',params: { id: row.id} })
      // let info = {
      //   size: 'large',
    ...
    edititem(row){ console.log('edititem(row)=',row);
        this.$router.push({name:'depositpaymentedit',params: { id: row.id} })
        // let edit = {
        //   title: 'รายละเอียดผลไม้',
        //   fields: { },
        //   callback: (values) => { 
    ...
    deleteitem(row){ 
      this.$confirm('คุณแน่ใจที่จะลบ รายการ นี้?', 'Warning', {
    ...
    additem() {
      this.$router.push({name:'depositpaymentnew'})
    },
    setting(){
                let setting = {
